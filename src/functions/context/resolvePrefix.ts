...
export const resolvePrefix = (
  ...
  guildPrefix: string | null,
) => {
  return (
    ctx.content.startsWith(ctx.client.prefix) ? ctx.client.prefix
    : ctx.content.startsWith(`<@${ctx.client.user!.id}>`) ? `${ctx.client.user}`
    : userPrefix && ctx.content.toLowerCase().startsWith(userPrefix) ? userPrefix
    : guildPrefix && ctx.content.toLowerCase().startsWith(guildPrefix) ? guildPrefix
    : noPrefix ? ""
    : "No results"
  );
};
